...
.factory('ProductsCombFac',function(ProductStockLiteFac,ProductsLiteFac,ProductsFac,UtilService,$rootScope,$http,$q,$filter,$cordovaSQLite)
{
    ...
    {
        var deferred        = $q.defer();
        ProductsLiteFac.GetProductUnits(parameters)
        .then(function(responselite)
        {
            if(angular.isArray(responselite) && responselite.length > 0)
            {
            ...
            else
            {
                ...
                .then(function(responseserver)
                {
                    if(angular.isArray(responseserver) && responseserver.length > 0 )
                    {
                        ...
                        angular.forEach(responseserver,function(value,key)
                        {
                            var promise = ProductsLiteFac.CreateProductUnits(value,'FROM-SERVER');
                            arraypromise.push(promise)
                        });
                        $q.all(arraypromise).then(function(result)
                        {
