...
function($rootScope,$q,$cordovaSQLite,UtilService)
{
    var SetProvinsis = function (datatosave)
    {
        ...
        var query           = 'INSERT INTO Tbl_RegProvinsis (PROVINCE_ID,PROVINCE) VALUES (?,?)';
    ...
    {
        var deferred  = $q.defer();
        var query     = 'SELECT * FROM Tbl_RegProvinsis';
    ...
    var SetKotas = function (datatosave)
    {
        ...
        var query           = 'INSERT INTO Tbl_RegKotas (PROVINCE_ID,PROVINCE,CITY_ID,CITY_NAME,TYPE,POSTAL_CODE) VALUES (?,?,?,?,?,?)';
        $cordovaSQLite.execute($rootScope.db,query,isitable)
        .then(function(result) 
        {
            deferred.resolve(result);
    ...
    {
        var deferred  = $q.defer();
        var query     = 'SELECT * FROM Tbl_RegKotas WHERE PROVINCE_ID=?';
