...
function(OfflineLiteFac,$rootScope,$q,$cordovaSQLite,UtilService)
{
    ...
    var GetProductImagesByIDLocal = function (ID_LOCAL)
    {
        var deferred    = $q.defer();
        var query       = 'SELECT * FROM Tbl_Product_Images WHERE PRODUCT_ID=?';
        $cordovaSQLite.execute($rootScope.db,query,[ID_LOCAL])
        .then(function(result) 
        {
            if(result.rows.length > 0)
    ...
    var UpdateProductGroups = function(datatoupdate,fromwhere)
    {
        var deferred        = $q.defer();
    ...
    {
        var deferred    = $q.defer();
        var query       = 'SELECT * FROM Tbl_Product_Units';
    ...
    var GetProductUnitsByUnitID = function (parameters)
    {
        var deferred    = $q.defer();
        var query       = 'SELECT * FROM Tbl_Product_Units WHERE UNIT_ID=?';
        $cordovaSQLite.execute($rootScope.db,query,[parameters.UNIT_ID])
        .then(function(result) 
        {
            if(result.rows.length > 0)
            {
    ...
    var GetProductUnitsByIDLocal = function (ID_LOCAL)
    {
        var deferred    = $q.defer();
        var query       = 'SELECT * FROM Tbl_Product_Units WHERE ID_LOCAL=?';
        $cordovaSQLite.execute($rootScope.db,query,[ID_LOCAL])
        .then(function(result) 
        {
            if(result.rows.length > 0)
    ...
    var CreateProductUnits = function(datatosave,fromwhere)
    {
        var deferred            = $q.defer();
        var UNIT_ID             = datatosave.UNIT_ID;
        var UNIT_NM             = datatosave.UNIT_NM;
        var UNIT_ID_GRP         = datatosave.UNIT_ID_GRP;
        var UNIT_NM_GRP         = datatosave.UNIT_NM_GRP;
        var STATUS              = datatosave.STATUS;
        var DCRP_DETIL          = datatosave.DCRP_DETIL;
        var CREATE_UUID         = datatosave.CREATE_UUID;
        var UPDATE_UUID         = datatosave.UPDATE_UUID;

        var isitable            = [UNIT_ID,UNIT_NM,UNIT_ID_GRP,UNIT_NM_GRP,STATUS,DCRP_DETIL,CREATE_UUID,UPDATE_UUID]
        var query               = 'INSERT INTO Tbl_Product_Units (UNIT_ID,UNIT_NM,UNIT_ID_GRP,UNIT_NM_GRP,STATUS,DCRP_DETIL,CREATE_UUID,UPDATE_UUID) VALUES (?,?,?,?,?,?,?,?)';
        $cordovaSQLite.execute($rootScope.db,query,isitable)
        .then(function(result) 
        {
            if(fromwhere != 'FROM-SERVER')
            {
                var dataoffline         = {};
                dataoffline.NAMA_TABEL  = 'Tbl_Product_Units';
                dataoffline.PRIMARY_KEY = result.insertId;
                dataoffline.TYPE_ACTION = 1;
                OfflineLiteFac.CreateOffline(dataoffline)
                .then(function(responsecreateoffline)
    ...
    var UpdateProductUnits = function(datatosave,fromwhere)
    {
        var deferred            = $q.defer();
        var UNIT_ID             = datatosave.UNIT_ID;
        var UNIT_NM             = datatosave.UNIT_NM;
        ...
        var isitable            = [UNIT_NM,UNIT_ID_GRP,UNIT_NM_GRP,STATUS,DCRP_DETIL,CREATE_UUID,UPDATE_UUID,UNIT_ID]
        var query               = 'UPDATE Tbl_UserOperasional SET UNIT_NM=?,UNIT_ID_GRP=?,UNIT_NM_GRP=?,STATUS=?,DCRP_DETIL=?,CREATE_UUID=?,UPDATE_UUID=? WHERE UNIT_ID=?';
        $cordovaSQLite.execute($rootScope.db,query,isitable)
        .then(function(result) 
        {
            if(fromwhere != 'FROM-SERVER')
    ...
    return{
            ...
            GetProductImagesByIDLocal:GetProductImagesByIDLocal,
            ...
            GetProductUnitsByUnitID:GetProductUnitsByUnitID,
            GetProductUnitsByIDLocal:GetProductUnitsByIDLocal,
            CreateProductUnits:CreateProductUnits,
