...
/** AWAL DARI PEMBAYARAN FUNCTION **/
    ...
    $scope.submitmodalpembayaran = function() 
    {
        function lanjutnextstep(datakartu)
        {
            ...
            parameters.TRANS_TYPE       = 0;//Produk Jual
    ...
    $scope.openmodalformcicilan = function()
    {
        if($scope.openbookdata.sudahopenbelum)
        {
            $ionicModal.fromTemplateUrl('templates/sales/cicilan/formcicilan.html', 
            {
                scope: $scope,
                animation: 'slide-in-left',
                backdropClickToClose: false,
            ...
            .then(function(modal) 
            {
                ...
                var resultcheckmodal = UtilService.CheckModalExistOrNot($scope.modalformcicilan);
                if(resultcheckmodal)
                {
                    $scope.modalformcicilan          = modal;
                    $scope.modalformcicilan.show();
                }
            });
        }
        else
    ...
    $scope.closemodalformcicilan = function()
    {
        $scope.modalformcicilan.remove();   
    }
...
/** AWAL DARI MENU PPOB **/
    ...
    $scope.openmodepembelian = function()
    {
        ...
        if($scope.openbookdata.sudahopenbelum)
        {
            $state.go('tab.sales-ppob', {}, {reload: false});
        }
        else
        {
            $scope.modalopenclosebookopen();
        ...
        if($scope.customerisexist.exist == 'false')
        {
            ...
            .then(function(responsenewcustomerid)
            {
                ...
                .then(function(responsesetcustomer)
                {
                    ...
                    ToastService.ShowToast('Pelanggan Baru Berhasil Didaftarkan','success');
                }
                ,function(error)
                {
                    ToastService.ShowToast('Pelanggan Baru Gagal Didaftarkan','success');
                });
            },
            function(error)
            {
...
/** AWAL DARI SAVE TO BILL FUNCTION **/
    $scope.openmodalsavetobill     = function()
    {
        ...
        .then(function(modal) 
        {
            ...
            $scope.mejaberapa       = {nomormeja:undefined};
        });
    }
        ...
        if($scope.itemincart.length > 0)
        {
            ...
            {                
                if(responsesavebill.length == 0)
                {
                    ...
                    parameters.NOMOR_MEJA = $scope.mejaberapa.nomormeja;
