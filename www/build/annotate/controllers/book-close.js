...
function(ToastService,OpenCloseBookCombFac,OpenCloseBookLiteFac,SummaryLiteFac,UtilService,ConstructorService,StorageService,$cordovaDialogs,$filter,$ionicLoading,$ionicHistory,$ionicModal,$timeout,$scope,$state)
{
    ...
    {
        var settingclosebook = StorageService.get('settingclosebook')
        if(settingclosebook)
        {
            $scope.viewtoshow  = {valuechoose:settingclosebook};   
        }
        else
        {
...
.controller('CloseBookDetailCtrl',['OpenCloseBookLiteFac','$stateParams','$scope','StorageService', 
function(OpenCloseBookLiteFac,$stateParams,$scope,StorageService)
{
    $scope.$on('$ionicView.beforeEnter', function(parameters)
    {
        console.log($stateParams.closebook_id);
        StorageService.set('settingclosebook','HISTORY');
